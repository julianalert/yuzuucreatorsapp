/**
 * Structural validator for blueprints.
 *
 * Deterministic, no model calls. Run before every model critic — it catches the
 * failures that would otherwise waste an expensive critic pass, and it is the
 * only gate that can prove coverage rather than estimate it.
 *
 * Two gates, not one:
 ...
 *
 * Usage:
 *   validateBlueprint(bp)                          // gate inferred from bp.status
 *   validateBlueprint(bp, { requireCoverage: true })
 */

const MODIFIER_TARGETS = ["constraints", "voice", "safety", "pacing"];

export function validateBlueprint(bp, opts = {}) {
  const errors = [];
  const warnings = [];
  const requireCoverage =
    opts.requireCoverage ?? ["complete", "approved"].includes(bp.status);
  const err = (path, msg, fix) => errors.push({ path, issue: msg, fix });
  const warn = (path, msg) => warnings.push({ path, issue: msg });
  ...
  for (const q of questions) {
    ...
    for (const d of q.drives ?? []) {
      if (!sections.includes(d)) {
        err(`quiz.questions.${q.id}.drives`,
    ...
    for (const m of q.modifies ?? []) {
      if (!MODIFIER_TARGETS.includes(m)) {
        err(`quiz.questions.${q.id}.modifies`,
  ...
  const emitted = new Map(); // signal -> Set of possible values
  for (const q of questions) {
  ...
  for (const rule of rules) {
    ...
    if (!conds.length) {
      err(`quiz.archetype_rules.${rule.id}`, "Rule has no match conditions.",
    ...
    for (const c of conds) {
      ...
      if (!possible) {
        err(`quiz.archetype_rules.${rule.id}`,
  ...
  const archetypeIds = [...rules.map((r) => r.id), bp.quiz?.fallback_archetype].filter(Boolean);
  ...
  if (missing.length) {
    ...
    if (requireCoverage) {
      err("output.content_bank", `Incomplete content bank — ${detail}.`,
  ...
  for (const [key, entry] of Object.entries(bank)) {
    for (const ref of entry.mechanism_refs ?? []) {
      if (!mechIds.has(ref)) {
        err(`output.content_bank.${key}.mechanism_refs`,
  ...
  for (const m of bp.knowledge_pack?.mechanisms ?? []) {
    if (!m.why_it_works?.trim()) {
      err(`knowledge_pack.mechanisms.${m.id}`,
  ...
  for (const t of bp.safety?.escalation_triggers ?? []) {
    if (t.block_id && !bank[t.block_id]) {
      err("safety.escalation_triggers",
  ...
  if (bp.status === "approved") {
    if (!bp.approved_at || !bp.approved_by) {
      err("status", "Marked approved without an approval record.",
 ...
 */
export function resolveArchetype(bp, answers) {
  const signals = {};
  for (const q of bp.quiz.questions) {
    ...
    for (const value of chosen) {
      ...
      for (const [k, v] of Object.entries(opt?.signals ?? {})) {
        ...
        else if (Array.isArray(signals[k])) signals[k].push(v);
        else signals[k] = [signals[k], v];
      }
    }
  }

  const has = (signal, wanted) => {
  ...
  const matches = bp.quiz.archetype_rules
    .filter((r) => {
      ...
      const test = (c) => has(c.signal, c.in ?? [c.equals]);
